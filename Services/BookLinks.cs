...
namespace Services
{
    public class BookLinks : IBookLinks
    {
            ...
            string fields, HttpContext httpContext, List<Entity> shapedBooks)
        {
            ...
            return new LinkResponse { HasLinks = true, LinkEntities = bookCollection };
        }
            ...
            BookDto bookDto, string fields)
        {
            var links = new List<Link>()
            {
                new Link()
                {
                    Href = $"/api/{httpContext
                    .GetRouteData().Values["controller"].ToString().ToLower()}" +
                    $"/{bookDto.Id}",
                    Rel = "self",
                    Method = "GET"
                },
                ...
                {
                    Href = $"/api/{httpContext
                    .GetRouteData().Values["controller"].ToString()}",
                    Rel = "create",
                    Method = "POST"
